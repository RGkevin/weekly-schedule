...
function ws_library($scheduleid = 1, $starttime = 19, $endtime = 22, $timedivision = 0.5, $layout = 'horizontal', $tooltipwidth = 300, $tooltiptarget = 'right center',
	...
	foreach ($daysoftheweek as $day)
	{
		for ($daysrow = 1; $daysrow <= $day->rows; $daysrow++)
		{
			...
			if ($items)
			{
				foreach($items as $item)
				{
					...
					$output .= '<td class="progress progress-striped active ws-item-' . $item->id . '" ';
     ...
     //                }
                    ...
                    $output .= '<div class="bar ws-item-title ws-item-title-' . $item->id . '"';

                    $output .= ' style="width: 100%;';

                    if ( !empty( $item->itemcolor) || !empty( $item->categorycolor) ) {

                        $output .= 'background-color:' . (!empty( $item->itemcolor) ? $item->itemcolor : $item->categorycolor ) . ';';
                    }

                    if ( !empty( $item->titlecolor ) )
                        $output .= 'color:' . $item->titlecolor . ';';
					...
					if ($displaydescription == "tooltip" && $item->description != "")
						$output .= "data-toggle=\"tooltip\" data-original-title='" . htmlspecialchars(stripslashes($item->description),  ENT_QUOTES) . "' ";

                    $output .= ' href="' . $item->address . '"';
                    $output .= '>' . stripslashes($item->itemname) . '</a>';
